...
subroutine SlowInjector
	if(t.lt.SlowDownInjectorAfter) return
	InjectorSpeed=SlowDownInjectorSpeed+0.99*abs(InjectorSpeed-SlowDownInjectorSpeed)
...
subroutine CoolParticle(coff)
	real(psn) :: coff ! The cooling cutoff speed/Lorentz factor
	real(psn) :: goff,gamma,r1,frac,gamma_new
	integer :: n 
	
	if(coff.lt.1.0) then
		goff=1.0_psn/sqrt((1.0_psn-coff)*(1.0_psn+coff))
	else
		goff=coff
	end if 
		
	    do n=1,used_prtl_arr_size !remove a random fraction of particl energy
		   gamma=sqrt(1.0_psn+up(n)**2+vp(n)**2+wp(n)**2)
		   if(gamma.gt.goff) then
			   call random_number(r1)
			   gamma_new= r1*(gamma-goff) + goff
			   frac=gamma_new/gamma
			   up(n)=up(n)*frac
			   vp(n)=vp(n)*frac
			   wp(n)=wp(n)*frac
		   end if

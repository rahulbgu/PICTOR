...
subroutine InitUser
		...
		!Ohter set-up specific initializations 
		xcond=4 !global x-cord of the conductor 
		PrtlWall=xcond+128! global x-cord of the Wall that reflects particles 
	    xinj=PrtlWall+10!+128!intial position, in global cordinates 
		BoxSizeIncrement=128*2
		...
		if(.not.restart) then 
		    ...
		    !call HomogeneousDriftingPrtlEqualWeightPUI(real(PrtlWall,psn),real(xinj,psn),DriftUpstream) 
		else			
...
subroutine ReflectNewPrtlRight(x1)
	...
	if(x1.le.mx) then 
		...
		do n=1,used_test_prtl_arr_size !remove any particle to the right of the injector 
			...
			if((qtp(n).ne.0).and.(xtp(n).ge.x1)) call ReflectPrtlMovingFrameX(DriftUpstream,utp(n),vtp(n),wtp(n))			
		end do 
	end if
...
subroutine InitdriftEFld(x1)
	real(psn):: x1
    integer :: i,j,k
	!Value of the electric field is always kept to be the -vxB0
    if(x1.le.mx) then
		 do k=1,mz
	          do j=1,my
	               do i=floor(x1-0.5_psn)+1,mx                  
	                    Ex(i,j,k)=driftEx		
				   end do
				   do i=floor(x1)+1,mx     
	                    Ey(i,j,k)=driftEy
...
subroutine InitReflectedPrtl
	implicit none
	integer   :: i,Nelc_New,tag
    real(psn) :: x1,x2,xglobal,r1
    real(psn):: xlocal,ylocal,zlocal,ugamma,vgamma,wgamma
    real(psn):: ubeta,vbeta,wbeta
 
 !Reflected particles 	
 x1=max(PrtlWall,xborders(procxind(proc)))
 x2=min(PrtlWall+RefCells,xborders(procxind(proc)+1))

 x1=x1-xborders(procxind(proc))+3 !change to local cordinate 
 x2=x2-xborders(procxind(proc))+3 
 
#ifdef twoD 
  Nelc_New=nint((x2-x1)*epc*(my-5))!First compute how many new particles are to be added 
#else 	 
  Nelc_New=nint((x2-x1)*epc*(my-5)*(mz-5))
#endif
  if(Nelc_New.gt.0) then 	
	 do i=1,Nelc_New
		!Electrons 
		...
		if(r1.lt.(xglobal-PrtlWall)/RefCells) then 
			cycle
		end if   		
		
		call GetVelGamma_MaxwellNonRel_StaticPDF(DriftUpstream,ugamma,vgamma,wgamma,GammaTableLength,GammaIon,Gamma_PDF_Ion)
		call InsertNewPrtl(xlocal,ylocal,zlocal,ugamma,vgamma,wgamma,1.0_psn-PUIratio,tag,1,time_this)	
       
		!ions 
		call GetTagID(tag,2)
		call GetVelGamma_MaxwellNonRel_StaticPDF(DriftUpstream,ugamma,vgamma,wgamma,GammaTableLength,GammaElc,Gamma_PDF_Elc)
		call InsertNewPrtl(xlocal,ylocal,zlocal,ugamma,vgamma,wgamma,-1.0_psn,tag,2,time_this)
 
	    !Pick up ions 
		if(PUIratio.gt.0) then 
			call GetTagID(tag,3)
			call GetVelGamma_MaxwellNonRel_StaticPDF(DriftUpstream,ugamma,vgamma,wgamma,GammaTableLength,GammaPUI,Gamma_PDF_PUI)
			call InsertNewPrtl(xlocal,ylocal,zlocal,ugamma,vgamma,wgamma,PUIratio,tag,3,time_this)
		end if 		 
	 end do	
  end if  
  
  !Particles between injector and the wall 
  x1=max(PrtlWall,xborders(procxind(proc)))
  x2=min(xinj,real(xborders(procxind(proc)+1)))
...
end subroutine InitReflectedPrtl

subroutine RefPrtlEfld
	real(psn) :: x1,x2,xglobal
	integer :: i,j,k,imin,imax
	
    x1=PrtlWall-xborders(procxind(proc))+3 !change to local cordinate 
    x2=PrtlWall+RefCells-xborders(procxind(proc))+3
	
	imin=max(1,floor(x1))
	imax=min(mx,floor(x2))
	
	 do k=1,mz
          do j=1,my
               do i=imin,imax 
				    xglobal=i+0.5_psn-3.0_psn+xborders(procxind(proc))                
                    Ex(i,j,k)=driftEx*(xglobal-PrtlWall)/real(RefCells)
				    xglobal=i-3.0_psn+xborders(procxind(proc))                
                    Ey(i,j,k)=driftEy*(xglobal-PrtlWall)/real(RefCells)
                    Ez(i,j,k)=driftEz*(xglobal-PrtlWall)/real(RefCells)
               end do
          end do
     end do
end subroutine RefPrtlEfld

!The following subroutines needs to be customized for every application 
!Note: Currently the subrroutine works only for left moving beam 
! subroutine RemoveWallPrtlFromUpstream
! 	integer, parameter :: TimeStepStart=10000
! 	integer, parameter :: TimeStepStop=100000
! 	integer :: Tmax
! 	integer :: n
! 	real(psn) :: Tau
! 	real(psn) :: prtlx_global,gbeta_drift
! 	if((t.lt.TimeStepStart).or.(t.gt.TimeStepStop)) return
! 	if(modulo(t,100).eq.0.or.(t.eq.TimeStepStart)) call FindShockXpos_BmagPeak(Xshock)
!
! 	Tmax=t-floor((xinj+Xshock)/abs(c*DriftUpstream))
! 	Tmax=max(Tmax,TimeStepStop)
!
! 	if(abs(DriftUpstream).gt.1.0_psn) then
! 		gbeta_drift=-sqrt((DriftUpstream-1.0_psn)*(DriftUpstream+1.0_psn))
! 	else
...
! 		do n=1,used_prtl_arr_size
! 			if(qp(n).eq.0) cycle
! 			prtlx_global=xp(n)+xborders(procxind(proc))-3
! 		    if((var1p(n).lt.Tmax).and.(prtlx_global.gt.XShock)) then ! to make sure that the particle is old enough
! 				Tau=0.5_psn*(prtlx_global-Xshock)/(xinj-Xshock)
! 				up(n)=gbeta_drift+Tau*abs(up(n)-gbeta_drift)
!                 vp(n)=Tau*vp(n)
! 				wp(n)=Tau*wp(n)
! 			end if
! 		end do
...
! 			if(qtp(n).eq.0) cycle
! 			prtlx_global=xp(n)+xborders(procxind(proc))-3
! 		    if((var1tp(n).lt.Tmax).and.(prtlx_global.gt.XShock)) then ! to make sure that the particle is old enough
! 				Tau=0.5_psn*(prtlx_global-Xshock)/(xinj-Xshock)
! 				utp(n)=gbeta_drift+Tau*abs(utp(n)-gbeta_drift)
!                 vtp(n)=Tau*vtp(n)
! 				wtp(n)=Tau*wtp(n)
! 			end if
! 		end do
!
! end subroutine RemoveWallPrtlFromUpstream
...
subroutine PostMovDep
    ...
    xinj_local=max(0.5_psn,xinj-xborders(procxind(proc))+3)
    call ReflectNewPrtlRight(xinj_local)  ! Reflect hot particles back into the box
...
!--------------------------------------------------------------------------------------------
!
!  The following Piece of code is helpful in exciting Shear Alfven waves to make the upsteam plasma turbulent 
!
!--------------------------------------------------------------------------------------------
...
subroutine GetCurrent(xglobal,yglobal,zglobal,ubeta,vbeta,wbeta,qm)
     ...
     if(qm.gt.0) then 
        ubeta=Ax*(qmi/qmi)/((qi*epc)*(1+me/mi)) !(Important: qmi/qm is changed to 1, becasue heavy particles get relatively large drift)
        vbeta=Ay*(qmi/qmi)/((qi*epc)*(1+me/mi))
        wbeta=Az*(qmi/qmi)/((qi*epc)*(1+me/mi))
   else
        ubeta=Ax*(me/mi)/((qe*epc)*(1+me/mi))
        vbeta=Ay*(me/mi)/((qe*epc)*(1+me/mi))
        wbeta=Az*(me/mi)/((qe*epc)*(1+me/mi))
   end if
...
subroutine InitEMfieldAlfvenWave(x1) 
     integer :: i,j,k
     real(dbpsn):: xglobal,yglobal,zglobal
     real(psn)  :: Ax,Ay,Az
	 real(psn)  :: x1
     if(x1.le.(mx-3.0_psn)) then
		 do k=1,mz
	          do j=1,my
	               do i=floor(x1-0.5_psn)+1,mx                   

...
!           exectime_grid=0
!           dXdomain=xborders(procxind+1)-xborders(procxind)
!    		  exectime_grid(procxind,procyind,proczind)=real(max(np,10))/dXdomain
...
!           end do
	...
	subroutine BalanceDomainSizeR(Den)
		...
		mx=xborders_new(procxind+1)-xborders_new(procxind)+5
        xmax=mx-2
        xlen=xmax-3

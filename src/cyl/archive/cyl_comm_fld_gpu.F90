...
contains
    subroutine ExchangeYZEdgeField_Axis_GPU(lFld_send,lFld_recv,rFld_send,rFld_recv)
	     ...
	     if(nSubDomainsX.lt.2) return
 		 if(procxind.eq.0) then
             do i=0,nSubDomainsY-1
		         ...
		         call MPI_RECV(rFld_recv(:,off+1:off+my_short,:),dcount1,mpi_psn,proc_grid(1,i,proczind),1,MPI_COMM_WORLD,stat,mpi_err)				 
		         call MPI_SEND(rFld_send(:,off+1:off+my_short,:),dcount2,mpi_psn,proc_grid(1,i,proczind),4,MPI_COMM_WORLD,stat,mpi_err)
			 end do 
		 end if 
		 if(procxind.eq.1) then
			 ...
			 dcount1=3*my_short*mz
	         dcount2=2*my_short*mz	 
		     call MPI_SEND(lFld_send(:,off+1:off+my_short,:),dcount1,mpi_psn,proc_grid(0,0,proczind),1,MPI_COMM_WORLD,stat,mpi_err)
			 call MPI_RECV(lFld_recv(:,off+1:off+my_short,:),dcount2,mpi_psn,proc_grid(0,0,proczind),4,MPI_COMM_WORLD,stat,mpi_err)
		 end if              
    ...
    subroutine ExchangeYZEdgeCurrent_Axis_GPU(lFld_send,lFld_recv,rFld_send,rFld_recv)
		  ...
		  if(procxind.eq.0) then
	          ...
	          do i=0,nSubDomainsY-1
		         ...
		         call MPI_RECV(lFld_recv(:,off+1:off+my_right,:),dcount1,mpi_psn,proc_grid(1,i,proczind),1,MPI_COMM_WORLD,stat,mpi_err)
		         call MPI_SEND(rFld_send(:,off+1:off+my_right,:),dcount1,mpi_psn,proc_grid(1,i,proczind),4,MPI_COMM_WORLD,stat,mpi_err)	
		 ...
		 if(procxind.eq.1) then
			 dcount1=3*my*mz
		     call MPI_SEND(lFld_send(1:3,:,:),dcount1,mpi_psn,proc_grid(0,0,proczind),1,MPI_COMM_WORLD,stat,mpi_err)
			 call MPI_RECV(lFld_recv,dcount1,mpi_psn,proc_grid(0,0,proczind),4,MPI_COMM_WORLD,stat,mpi_err)
			 if(procyind.ne.0) then ! avoid repetition in ghost cells during the exchange of XZ and XY edges

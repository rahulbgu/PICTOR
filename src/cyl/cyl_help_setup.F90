...
!---------------------------------------------------------------------------------		
    subroutine InitElectricField_cyl(Efld)
        ...
        do j=1,my 
		    do i=i1,mx
				r=i -3.0_psn + rborders(procxind(proc))
				theta= 2*pi*(j -3.0_psn + yborders(procyind(proc)))/ny
#ifdef twoD    
                z=0.0_psn
#else				 				
...
! 		end if
    ...
    subroutine InitMagneticField_cyl(Bfld)
		...
		if(procxind(proc).eq.0) call UpdateBFldAxis 
	...
	subroutine InitPrtlMom_cyl(Flvr,Temperature,DriftVelocity)
		...
		do i=initialised_prtl_ind+1,prtl_arr_size
			if((qp(i).ne.0).and.flvp(i).eq.Flvr) then  
				r=xp(i)-xmin+rborders(procxind(proc))
				theta=(yp(i)-ymin+yborders(procyind(proc)))*dtheta
				z=zp(i)-zmin+zborders(proczind(proc))
	...
	!the new version first estiamtes prtl arr size and then allocates the prtl arr and then intialise the particles 
 	subroutine InitPrtlPos_cyl(ch,flv,Den,max_den)
	    ...
	    rmin_global2=rborders(procxind(proc))**2
	    rmax_global2=rborders(procxind(proc)+1)**2
		if(procxind(proc).eq.0) rmin_global2=0
		
		yglobal_max = yborders(procyind(proc)+1)
		if(inc_axis) then
			if(procxind(proc).eq.0) yglobal_max=ny
		end if
		...
		do i=0,Nelc_uniform_cyl_this-1,init_block
			...
			do j=1,init_block_this
				...
				yglobal= yborders(procyind(proc)) + rnd2*(yglobal_max-yborders(procyind(proc)))
				zglobal= zborders(proczind(proc)) + rnd3*(zborders(proczind(proc)+1)-zborders(proczind(proc)))
				theta=yglobal*dtheta
				if(rnd_acpt.le.Den(sqrt(r_global2),theta,zglobal)/max_den) then
					 xlocal=sqrt(r_global2)-rborders(procxind(proc))+xmin

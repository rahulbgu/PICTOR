...
contains 
	subroutine MoveDepositPrtlGPU
		...
		!if((inc_axis.eq..true.).and.(procxind(proc).eq.0)) then
		if((inc_axis.eq..true.).and.(procxind(proc).le.0)) then
			...
			do kc=1,Nchunk_prtl_gpu
				...
				indf=(kc-1)*chunk_size_prtl_gpu+used_prtl_chunk(kc)
			       call MoveDepositAxisPrtlKernel<<<ceiling(real(used_prtl_chunk(kc))/NthreadsGPU), NthreadsGPU>>>(prtl_arr_size,xp_gpu,yp_gpu,zp_gpu,up_gpu,vp_gpu,wp_gpu,flvp_gpu,qp_gpu,tagp_gpu,flvrqm_gpu,c,sqc,cinv,qi,&
			                     TexEx_gpu,TexEy_gpu,TexEz_gpu,TexBx_gpu,TexBy_gpu,TexBz_gpu,Jx_gpu,Jy_gpu,Jz_gpu,mx,my,mz,indi,indf,rborders(procxind(proc))-xmin,inv_dtheta,ymin,ymax,ylen,ny)
			...
			do kc=1,Nchunk_prtl_gpu
				...
				indf=(kc-1)*chunk_size_prtl_gpu+used_prtl_chunk(kc)
			       call MoveDepositPrtlKernel<<<ceiling(real(used_prtl_chunk(kc))/NthreadsGPU), NthreadsGPU>>>(prtl_arr_size,xp_gpu,yp_gpu,zp_gpu,up_gpu,vp_gpu,wp_gpu,flvp_gpu,qp_gpu,tagp_gpu,flvrqm_gpu,c,sqc,cinv,qi,&
			                     TexEx_gpu,TexEy_gpu,TexEz_gpu,TexBx_gpu,TexBy_gpu,TexBz_gpu,Jx_wide_gpu,Jy_wide_gpu,Jz_wide_gpu,mx,my,mz,indi,indf,rborders(procxind(proc))-xmin,inv_dtheta,Jwidth_gpu)
	...
	attributes(global) subroutine MoveDepositPrtlKernel(size,xp,yp,zp,up,vp,wp,flvp,qp,tagp,flvrqm,c,sqc,cinv,qi,Ex,Ey,Ez,Bx,By,Bz,Jx,Jy,Jz,mx,my,mz,indi,indf,r_proc,inv_dtheta,jwidth)
	    integer, value :: size

...
!-----------------------------------------------------------------------
     ...
     subroutine CopyTaggedPrtlFromGPU
		 integer :: kc, indi, indf

	      do kc=1,Nchunk_prtl_gpu
	 		   np_recv_gpu=0
	 		   np_send_gpu=np_recv_gpu
			   
   		       indi=(kc-1)*chunk_size_prtl_gpu+1
   		       indf=(kc-1)*chunk_size_prtl_gpu+used_prtl_chunk(kc)
			
			   call CollectPrtlGPUKernel<<<ceiling(real(used_prtl_chunk(kc))/NthreadsGPU), NthreadsGPU>>>(qp_gpu,xp_gpu,yp_gpu,zp_gpu,up_gpu,vp_gpu,wp_gpu,var1p_gpu,tagp_gpu,flvp_gpu,indi,indf,np_send_gpu,(empty_prtl_chunk-1)*chunk_size_prtl_gpu)
			   
			   np_recv_gpu = np_send_gpu
     ...
     end subroutine CopyTaggedPrtlFromGPU
	 
	 

	 
	 
	 attributes(global) subroutine CollectPrtlGPUKernel(q,x,y,z,u,v,w,var1,tag,flv,i1,i2,index,off)
...
!   		  nprtl_save_this=tosave_prtl_size_host

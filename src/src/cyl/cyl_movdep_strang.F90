...
contains 
     ...
     subroutine MoveDepositPrtl
		  ...
		  real(psn), dimension(VecBlockSize) :: x1,y1,r1, r0
		  integer, dimension(VecBlockSize):: ip0,jp0,kp0
		  integer, dimension(VecBlockSize):: ip,jp,kp
		  real(psn)    :: xr,yr,zr
		...
		do n=1,VecBlockSizeThis 
		    if(flvp(n+off).eq.0) then ! if condition can possibly be removed by defining flvrqm(0)=0 
			  qm(n)=0
  		      ...
  		      qm(n)=flvrqm(flvp(n+off))*0.25_psn !for optimisation 0.5 is multiplied here itself 
		    end if 
		end do 
...
!$OMP SIMD
       ...
       !Strang operator splitting method -- Delzanno et al JCP 253 (year:2013)
	   
!$OMP SIMD
       do n=1,VecBlockSizeThis   
		       ! first half of the move ,  acceleration
			   ...
			   r1(n)=1.0_psn/r1(n)
			   u1(n)=(r0(n)*u0(n) +   (u0(n)*u1(n)*g(n)+v0(n)*y1(n))  )*r1(n)
	           v1(n)=(v0(n)*r0(n))*r1(n)
...
#ifdef twoD 
               short_arr_ind= (jp(n)-1)*mx + ip(n) 
#else
               short_arr_ind= (kp(n)-1)*mx*my  + (jp(n)-1)*mx + ip(n) 
#endif
...
!$OMP SIMD		   
        ...
        do n=1,VecBlockSizeThis
              ...
              kp=1
#endif
                xr=min(real(min(ip0(n),ip(n))+1,psn),max(real(max(ip0(n),ip(n)),psn),0.5_psn*(x0(n)+xp(n+off))))
                yr=min(real(min(jp0(n),jp(n))+1,psn),max(real(max(jp0(n),jp(n)),psn),0.5_psn*(y0(n)+yp(n+off))))
                zr=min(real(min(kp0(n),kp(n))+1,psn),max(real(max(kp0(n),kp(n)),psn),0.5_psn*(z0(n)+zp(n+off))))
...
!$OMP SIMD
		        ...
		        Wx2=0.5_psn*(xp(n+off)+xr)-ip(n)
		        Wy2=0.5_psn*(yp(n+off)+yr)-jp(n)
#ifdef twoD
		        Wz2=0.0_psn
				short_arr_ind= (jp0(n)-1)*mx + ip0(n)
#else
		        Wz2=0.5_psn*(zp(n+off)+zr)-kp(n)
				short_arr_ind= (kp0(n)-1)*mx*my  + (jp0(n)-1)*mx + ip0(n)
#endif
...
! Periodic Boundary conditions 
	 ...
	 subroutine MovePrtlAxis(n,x,y,z,u,v,w,dt,res)
		 ...
		 real(psn) :: qm
		 real(psn) :: u0,v0,w0,u1,v1,w1,f,g,x1,y1,r1,dy,r0
		 ...
		 if(flvp(n).eq.0) then ! if condition can possibly be removed by defining flvrqm(0)=0 
			  qm=0
  		      ...
  		      qm=flvrqm(flvp(n))*0.25_psn !for optimisation 0.5 is multiplied here
		 end if 
         pEx=pEx*qm*dt
         pEy=pEy*qm*dt
         pEz=pEz*qm*dt
         ...
         yp(n)=yp(n)+dy
		 zp(n)=zp(n) + w1*g*dt*grid_inv_dz
		 	   
		 call Interp_VecEM_GridPoints(xp(n),yp(n),zp(n),pEx,pEy,pEz,pBx,pBy,pBz)

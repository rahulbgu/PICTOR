...
!---------------------------------------------------------------------------	
	...
	subroutine NumCells1D(np1d_local,ncells)
		real(dbpsn), dimension(:) :: np1d_local
		integer :: i, ncells, f
		...
		do i=1,size(np1d_local)
			np1d_local(i) = np1d_local(i) + f*ncells
		end do
		
	end subroutine NumCells1D
	
	subroutine BndOverhead(np1D_local)
		real(dbpsn), dimension(:) :: np1d_local
		integer :: f1, f2
		
		if(kproc.eq.0 .or. kproc.eq. ProcGrid(iproc,jproc)%count -1	) then
			f1 = 2*indepLBaxis +1  
			f2 = 2*indepLBaxis +2
			
			if( bc_face(f1)%type_prtl .ne. 'prdc' .or.  bc_face(f2)%type_prtl.ne. 'prdc') then 
				np1D_local = 1.5*np1D_local 
			end if
		
		end if
		
	end subroutine BndOverhead
	...
	subroutine AdjustIndepBorders
		...
		!count the number of particles in cells along indepLBaxis
		...
		!include the number of cells, useful if ppc is low
		if(indepLBaxis.eq.0) call NumCells1D(np1D_local,my*mz)
		if(indepLBaxis.eq.1) call NumCells1D(np1D_local,mx*mz)
		if(indepLBaxis.eq.2) call NumCells1D(np1D_local,mx*my) 
		
		!include boundary cells
		call BndOverhead(np1D_local)
		...
		if(proc.eq.0) print*,proc,'previous borders are:',ProcGrid(iproc,jproc)%borders,'time',t
...
!if(procyind.eq.0) borders_new(1) = 12
		...
		if( borders_changed(ProcGrid(iproc,jproc)%count,ProcGrid(iproc,jproc)%borders,borders_new) ) then
			...
			call SetNewBorders(ProcGrid(iproc,jproc)%count,ProcGrid(iproc,jproc)%borders,ProcGrid(iproc,jproc)%procs,ProcGrid(iproc,jproc)%count,borders_new,ProcGrid(iproc,jproc)%procs)
	...
	subroutine ResetCommSubdomainsLB
		call SetNgbr
		...
		if(proc.eq.1) then 
			print*,'borders:',ProcGrid(iproc,jproc)%borders
			print*,'mx my mz', mx, my , mx
	...
	end subroutine ResetCommSubdomainsLB
	...
	!--------------------------------------------------------------------------
	subroutine SetNewBorders(size,borders,procs,size_new,borders_new,procs_new)
		...
		integer, dimension(0:size_new-1) :: procs_new
		...
		call SetCommSkeltonLB(size,borders,procs,size_new,borders_new,procs_new)
		...
		call SetNewFldLB(Ex,Ey,Ez,size,borders,size_new,borders_new)
		...
		call SetNewFldLB(Bx,By,Bz,size,borders,size_new,borders_new)


		!Resize PML arrays
		if(pmlBC) then 
			call SetNewPMLFldLB(pml_E_psi1%x,pml_E_psi1%y,pml_E_psi1%z,size,borders,size_new,borders_new)
			call SetNewPMLFldLB(pml_E_psi2%x,pml_E_psi2%y,pml_E_psi2%z,size,borders,size_new,borders_new)
			call SetNewPMLFldLB(pml_B_psi1%x,pml_B_psi1%y,pml_B_psi1%z,size,borders,size_new,borders_new)
			call SetNewPMLFldLB(pml_B_psi2%x,pml_B_psi2%y,pml_B_psi2%z,size,borders,size_new,borders_new)
		end if 
	...
	subroutine SetNewFldLB(Fldx,Fldy,Fldz,size,borders,size_new,borders_new)
		integer :: size, size_new
		integer, dimension(0:size)               :: borders
		integer, dimension(0:size_new)           :: borders_new
		...
		call WaitCommBuffFlds(lb_send)
		call WaitCommBuffFlds(lb_recv)		
		call CopyFldsToBuffNgbrs(Fldx,Fldy,Fldz,lb_send)
		call SendBuffFlds(lb_send)
		call ResizeFldsLB(Fldx,size,borders,size_new,borders_new,indepLBaxis,kproc)
		call ResizeFldsLB(Fldy,size,borders,size_new,borders_new,indepLBaxis,kproc)
		call ResizeFldsLB(Fldz,size,borders,size_new,borders_new,indepLBaxis,kproc)
		call RecvBuffFlds(lb_recv)
		call WaitCommBuffFlds(lb_recv)
		call CopyBuffToFldsNgbrs(Fldx,Fldy,Fldz,lb_recv)
	
	end subroutine SetNewFldLB 
    
	!only the load balacing along the x-axis is implemented in case of PML
	subroutine SetNewPMLFldLB(Fldx,Fldy,Fldz,size,borders,size_new,borders_new)
		integer :: size, size_new
		real(psn), dimension(:,:,:), allocatable :: Fldx, Fldy, Fldz
		integer, dimension(0:size)       :: borders
		integer, dimension(0:size_new)   :: borders_new
		...
		if(.not.allocated(Fldx)) then
			allocate(Fldx(mx,my,mz), Fldy(mx,my,mz), Fldz(mx,my,mz))
			Fldx = 0.0_psn; Fldy=0.0_psn; Fldz=0.0_psn
		end if

		call SetNewFldLB(Fldx,Fldy,Fldz,size,borders,size_new,borders_new)
		
		
		if(kproc+1.gt.size_new) then 
			if(allocated(Fldx)) deallocate(Fldx, Fldy, Fldz)
			return
		end if
		
		mx_new = borders_new(kproc+1) - borders_new(kproc)+5
		if(pml_outside_domain(mx_new,my,mz,size_new,nSubDomainsY,nSubDomainsZ,borders_new,yborders,zborders)) then 
			if(allocated(Fldx)) deallocate(Fldx, Fldy, Fldz)
		end if 
		
	end subroutine SetNewPMLFldLB 
	
	subroutine SetCommSkeltonLB(size,borders,procs,size_new,borders_new,procs_new)
		integer :: size, size_new
		integer, dimension(0:size)       :: borders
		integer, dimension(0:size_new)   :: borders_new
		integer, dimension(0:size-1)     :: procs
		...
		call MultipleNgbr( size, borders, size_new , borders_new, procs_new , lb_send(1))
		call MultipleNgbr( size_new, borders_new, size, borders, procs , lb_recv(1))
	...
	subroutine ResizeFldsLB(Fld,size,borders,size_new,borders_new,axis,procind)
		real(psn), dimension(:,:,:), allocatable :: Fld
		real(psn), dimension(:,:,:), allocatable :: Fld_new
		integer :: procind, axis
		integer :: size, size_new
		integer, dimension(0:size)     :: borders
		integer, dimension(0:size_new) :: borders_new 
		integer :: p1, p2
		...
		if(procind+1.gt.size_new) return
		...
		if(procind+1.le.min(size,size_new)) then 
		...
		Fld_new = 0.0_psn
		if(procind+1.le.min(size,size_new)) then
			 Fld_new(ind_new(1):ind_new(2),ind_new(3):ind_new(4),ind_new(5):ind_new(6)) = Fld(ind_old(1):ind_old(2),ind_old(3):ind_old(4),ind_old(5):ind_old(6))
		end if
	...
	subroutine LoadPrtlOutlierLB(axis,list,edges)
		...
		do n=1,used_prtl_arr_size
            ...
            call CopyTransferPrtl(qp,xp,yp,zp,up,vp,wp,var1p,flvp,tagp,procp,qmp,wtp,list,n)
